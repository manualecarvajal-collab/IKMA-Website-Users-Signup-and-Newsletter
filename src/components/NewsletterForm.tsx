  ...
  if (isSuccess) {
    return (
      <div className="p-6 md:p-8 max-w-5xl mx-auto text-center py-24">
        ...
        <h2 className="font-headline-lg text-headline-lg text-primary mb-2">{t("newsletterSent") || "Newsletter Sent!"}</h2>
        ...
        >
          {t("backToNewsletter") || "Back to Newsletter"}
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-headline-lg text-headline-lg text-primary">{heading || t("newNewsletter")}</h1>
          {headingDescription && (
            <p className="font-body-md text-body-md text-on-surface-variant mt-1">{headingDescription}</p>
          )}
        </div>
        <button
        ...
        >
          ...
          {showPreview ? t("edit") || "Edit" : t("preview") || "Preview"}
        </button>
      </div>
      ...
      {showPreview ? (
        <div className="bg-surface rounded-xl border border-outline-variant/20 overflow-hidden">
          <div className="p-4 bg-surface-container-low border-b border-outline-variant/20">
            <span className="font-label-bold text-label-bold text-on-surface-variant">{t("emailPreview") || "Email Preview"}</span>
          </div>
          <iframe
            srcDoc={previewHtml}
        ...
        <>
          ...
          <form ref={formRef} action={action} className="space-y-6">
            {id && <input type="hidden" name="newsletter_id" value={id} />}
            ...
            <div className="bg-surface rounded-xl p-6 border border-outline-variant/20 space-y-6">
              ...
              <div>
                <label className="block font-label-bold text-label-bold text-on-surface mb-2">
                  {t("bannerImage") || "Banner Image (optional)"}
                </label>
                <button
                  type="button"
                  onClick={() => {
                ...
                >
                  ...
                  {imagenUrl ? t("changeImage") || "Change Image" : t("addImage") || "Add Image"}
                </button>
                {imagenUrl && (
            ...
            <div className="flex gap-3">
              ...
              >
                {pending ? t("sending") || "Sending..." : scheduledAt ? t("scheduleForLater") || "Schedule" : t("sendToSubscribers") || "Send to Subscribers"}
              </button>
            </div>
          </form>
