  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-headline-lg text-headline-lg text-primary">{t("magazines")}</h1>
          <p className="font-body-md text-body-md text-on-surface-variant">{t("magazinesDesc")}</p>
        </div>
        <Link
        ...
        >
          <Icon name="add" size={14} /> {t("newMagazine")}
        </Link>
      </div>
      ...
      <div className="bg-surface-container-lowest rounded-xl border border-outline-variant/10 overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {revistas?.length === 0 && (
              <tr><td colSpan={4} className="px-6 py-12 text-center font-body-md text-body-md text-on-surface-variant">{t("noMagazines")}</td></tr>
            )}
            {revistas?.map((r) => (
